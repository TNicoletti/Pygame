...
#lg.marcarVisto()
bg = (122, 48, 72)
#enemies.append(Enemy(790 - 75, 10, player, obstaculo, size, bullets))
#enemies.append(MagicWarrior1(10, 10, player, obstaculo, size, bullets))
#enemies.append(MagicWarrior1(790 - 75, 10, player, obstaculo, size, bullets))
#enemies.append(Patocomarma(10, 10, player, obstaculo, size, bullets))
#enemies.append(Patocomarmaebandana(10, 10, player, obstaculo, size, bullets))
enemies.append(TheFirstBoss(400 - 50, 400 - 50, player, obstaculo, size, bullets))
...
while(run):
    ...
    if (len(enemies) == 0):
        if(lg.getAtualMap().tipo==3):
            ...
            lg = levelGenerator(player, bullets, seed, 10+(andar*2), size)
            enemies = lg.getAtualMap().enemies
            obstaculo = lg.getAtualMap().obstaculos
            player.platforms = obstaculo
            shops = lg.getAtualMap().shops
    ...
    for x in bullets:
        if (x.x < 0 or x.x > size or x.y < 0 or x.y > size):
            toRemove.append(x)
            continue

        if (player.confereMargem(x)):
            player.takeDamage(x.damage)
            toRemove.append(x)
            continue
